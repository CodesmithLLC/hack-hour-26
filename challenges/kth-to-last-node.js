...
function kthToLastNode(k, head) {
  // need to check if n > 0;
  if (k < 1) return;
  const values = [];
  let currentNode = head;
  // need to traverse the list, storing values of each node in an array
  while (currentNode.next) {
    values.push(currentNode.value);
    currentNode = currentNode.next;
  }
  // while loop quits before the value of the tail is pushed, so do that next
  values.push(currentNode.value);
  // console.log(values);
