...
function getPINs(observed) {
  ...
  function buildCombos(pin = [], result = '') {
    if (pin.length === 0) return allPINs.add(result);
    const num = pin.shift();
    const posbilities = adjacent[num];
    posbilities.forEach(number => buildCombos(pin, result + number));
  }
