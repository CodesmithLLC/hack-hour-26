...
function hasCycle(head) {
  ...
  while (!nodes.has(current)) {
    // found the tail -> no cycles -> return false
    if (current.next === null) return false;

    // not at the tail yet, keep adding to nodes and traverse
    nodes.add(current);
    current = current.next;
  }

  // broke out of loop -> nodes has a reference we've already visited
  return true;
}
...
node5.next = node2;
console.log(`true: ${hasCycle(node1)}`);
